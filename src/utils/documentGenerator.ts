...
export async function generatePaymentPlanPDF(
  ...
  const GREEN  = [0, 108, 74]  as const;  // #006c4a
  ...
  // Logo centrado: imagen real o fallback a texto
  let y = 10;
  if (logoInfo) {
    const maxLogoW = 70;
    const maxLogoH = 22;
  ...
  const drawStage = (
    ...
    // Para etapas sin tabla (ej. Reserva): mostrar total en el encabezado
    if (base === null) {
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
  ...
  // -- Texto de aval (posicion fija, siempre encima del footer) --
  const avalY = 297 - 38;
...
export interface ContractData {
  ...
  // Específico para Reserva y Compraventa
  importeReserva?: number;
  porcentajeIva?: number; // p. ej. 10
  ...
  // Específico para Entrega de Llaves
  juegosLlaves?: number;
  mandosGaraje?: number;
  lecturaAgua?: string;
  lecturaLuz?: string;
...
function formatEur(n: number = 0): string {
...
function numeroALetras(n: number = 0): string {
 ...
 */
export function generatePDF(data: ContractData, download: boolean = true): Blob {
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
  const pageW = 210;
  const margin = 20;
  const contentW = pageW - margin * 2;
...
export async function generateDOCX(data: ContractData): Promise<void> {
  try {
    ...
    if (!response.ok) {
      // Fallback a generación de archivo de texto enriquecido si no hay plantilla .docx previa
      const docxContent = `
=====================================================
